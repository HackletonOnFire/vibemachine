...
export class PromptEngineeringService {
  ...
  private createCoreSustainabilityTemplate(): PromptTemplate {
    return {
...
Focus on recommendations that align with {industry} best practices for {companySize} companies and can realistically be implemented within typical corporate approval processes.`,
  ...
  private createTechnologyTemplate(): PromptTemplate {
    return {
...
Include technology industry benchmarks and consider typical IT budgets and approval processes.`,
  ...
  private createManufacturingTemplate(): PromptTemplate {
    return {
...
- Account for industrial safety requirements`,
  ...
  private createRetailTemplate(): PromptTemplate {
    return {
...
- Consider seasonal energy variations`,
  ...
  private createHealthcareTemplate(): PromptTemplate {
    return {
...
- Can be implemented with minimal operational disruption`,
  ...
  private createSmallBusinessTemplate(): PromptTemplate {
    return {
...
- Don't disrupt daily operations`,
  ...
  private createEnterpriseTemplate(): PromptTemplate {
    return {
      id: 'enterprise_v1',
...
- Include competitive benchmarking and industry leadership opportunities`,
  ...
  private createCarbonNeutralTemplate(): PromptTemplate {
    return {
...
- Verification and reporting frameworks`,
  ...
  private createCostOptimizationTemplate(): PromptTemplate {
    return {
...
Include detailed financial analysis with sensitivity scenarios for energy price changes.`,
  ...
  private createCoreSustainabilityExamples(): FewShotExample[] {
    return [
      {
        input: {
  ...
  public selectOptimalPrompt(request: RecommendationRequest): PromptTemplate {
    ...
    if (goalTemplate) return goalTemplate;
    if (industryTemplates.length > 0) return industryTemplates[0];
    if (sizeTemplate) return sizeTemplate;
    
    return this.templates.get('sustainability_core_v2')!;
  }
  
  private getIndustryTemplates(industry: string): PromptTemplate[] {
  ...
  private getSizeTemplate(companySize: string): PromptTemplate | null {
  ...
  private getGoalTemplate(goals: string[]): PromptTemplate | null {
  ...
  public generatePrompt(request: RecommendationRequest): { systemPrompt: string; userPrompt: string; template: PromptTemplate } {
  ...
  public recordMetrics(promptId: string, metrics: Partial<PromptMetrics>): void {
    if (!this.metrics.has(promptId)) {
      this.metrics.set(promptId, []);
    }
    
    const fullMetrics: PromptMetrics = {
      promptId,
  ...
  public getTemplatePerformance(templateId: string): { avgQuality: number; avgSatisfaction: number; usageCount: number } {
  ...
  public listAvailableTemplates(): Array<{ id: string; name: string; version: string; industries: string[] }> {
